...
The `services` package holds orchestration logic and most cross-cutting runtime behavior. The matching [`CLAUDE.md`](CLAUDE.md) file mirrors the same local rules for Claude Code.
...
- [`claude_runner.py`](claude_runner.py): Anthropic provider CLI execution, streaming, retry classification, and JSON parsing
...
- Keep [`claude_runner.py`](claude_runner.py) thin and provider-CLI-focused. Business rules belong in the caller.
