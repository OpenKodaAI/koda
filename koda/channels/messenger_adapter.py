"""Facebook Messenger adapter using direct HTTP to the Graph API.

Transport: Webhook — the adapter exposes ``handle_webhook`` and
``handle_webhook_verification`` for aiohttp route registration.
No external SDK dependency; uses aiohttp for HTTP calls.
"""
...
class MessengerAdapter(ChannelAdapter):
    """Bridges Facebook Messenger via the Graph API into the Koda channel layer."""
    ...
    async def initialize(self, agent_id: str, secrets: dict[str, str]) -> None:
        ...
        self._page_access_token = secrets["MESSENGER_PAGE_ACCESS_TOKEN"]
        self._verify_token = secrets["MESSENGER_VERIFY_TOKEN"]
        self._app_secret = secrets["MESSENGER_APP_SECRET"]
        log.info("messenger.initialized: agent=%s", agent_id)
    ...
    async def start(self) -> None:
        ...
        self._running = True
        log.info("messenger.started: agent=%s", self._agent_id)
    ...
    async def stop(self) -> None:
        ...
        if self._session:
            ...
            self._session = None
        log.info("messenger.stopped: agent=%s", self._agent_id)
    ...
    async def handle_webhook_verification(self, request: Any) -> Any:
        ...
        if mode == "subscribe" and token == self._verify_token:
            log.info("messenger.webhook_verified: agent=%s", self._agent_id)
            return web.Response(text=challenge, content_type="text/plain")

        log.warning("messenger.webhook_verify_failed: agent=%s", self._agent_id)
        return web.Response(status=403, text="Verification failed")

    async def handle_webhook(self, request: Any) -> Any:
        """Process inbound Messenger webhook POST."""
        from aiohttp import web
        ...
        if not self._verify_signature(body_bytes, signature):
            log.warning("messenger.invalid_signature: agent=%s", self._agent_id)
