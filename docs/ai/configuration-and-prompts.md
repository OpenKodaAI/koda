...
[`../../koda/control_plane/settings.py`](../../koda/control_plane/settings.py) owns control-plane settings (rate limits, session TTL, master-key files). It also hosts first-run auth policy: `KODA_ENV`, `ALLOW_LOOPBACK_BOOTSTRAP`, `CONTROL_PLANE_BOOTSTRAP_CODE`, `CONTROL_PLANE_OPERATOR_PASSWORD_MIN_LENGTH` (default 12), and `CONTROL_PLANE_RECOVERY_CODES_PER_USER` (default 10). `KODA_ENV=production` hard-fails when `ALLOW_LOOPBACK_BOOTSTRAP=true`. The control-plane backend always enforces an operator-session bearer token at the middleware layer (with a fixed public whitelist for onboarding, status, and login routes).
...
- `PROVIDER_FALLBACK_ORDER` defines an operator-configured chain of providers to try when the primary provider cannot start or continue a task. Empty by default — the operator opts in via the control-plane `models.fallback_order` setting. Without explicit configuration the runtime does not silently fall through to other providers.
...
- `CONTROL_PLANE_API_TOKEN` is the CLI bootstrap credential used by `koda setup-code` to request a setup code from the control plane over loopback. The browser path is setup-code exchange, local owner login, and an HTTP-only session. The legacy `/api/control-plane/web-auth` compatibility bridge has been removed; CLI bootstrap flows use `/api/control-plane/auth/bootstrap/codes` with an `Authorization: Bearer $CONTROL_PLANE_API_TOKEN` header.
