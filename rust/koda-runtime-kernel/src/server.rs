use crate::agent_workers;
use crate::browser::{
    open_browser_log, resolve_binary_path, stop_managed_browser_session,
    ManagedBrowserSessionHandle,
};
use crate::checkpoints::{
    build_checkpoint_archive_path, build_untracked_bundle_bytes, git_diff_bytes,
    git_head_commit_sha, restore_checkpoint_bundle, run_git_output, RestoreCheckpointOutcome,
};
use crate::commands::execute_shell_command;
use crate::processes::{
    default_shell_args, default_shell_command, prepare_process_command, send_process_signal,
};
use crate::state::{
    now_ms, BrowserSessionRecord, BrowserSessionSnapshot, CheckpointRecord, CheckpointSnapshot,
    EnvironmentRecord, EnvironmentSnapshot, InteractiveTerminalRecord, InteractiveTerminalSnapshot,
    KernelSnapshot, KernelState, SnapshotPayload, TaskRecord, TaskSnapshot, TerminalRecord,
    TerminalSnapshot,
};
use crate::terminals::TerminalRegistry;
use crate::workspace::{cleanup_workspace, provision_workspace, ProvisionedWorkspace};
...
const AUTHORITATIVE_OPERATIONS: &str =
    "create_environment,start_task,execute_command,attach_terminal,stream_terminal,open_terminal,write_terminal,resize_terminal,close_terminal,stream_terminal_session,pause_task,resume_task,terminate_task,finalize_task,cleanup_environment,start_browser_session,stop_browser_session,get_browser_session,save_checkpoint,get_checkpoint,restore_checkpoint";
...
pub struct RuntimeKernelServer {
    ...
    checkpoint_locks: Arc<Mutex<HashMap<String, Arc<Mutex<()>>>>>,
...
impl RuntimeKernelServer {
    ...
    pub fn with_agent_worker_adapter(
        ...
        Self {
            ...
            terminal_registry: TerminalRegistry::new(TERMINAL_HISTORY_RECORD_LIMIT),
    ...
    fn append_terminal_bytes(
        ...
        push_terminal_record(&mut state.terminal_history, task_id, record.clone());
        terminal_registry.append(task_id, record);
    ...
    async fn checkpoint_workspace_lock(
        &self,
        workspace_path: &str,
        fallback_key: &str,
    ) -> Arc<Mutex<()>> {
        let key = if workspace_path.is_empty() {
            fallback_key.to_string()
        } else {
            workspace_path.to_string()
        };
        let mut locks = self.checkpoint_locks.lock().await;
        locks
            .entry(key)
...
fn append_interactive_terminal_bytes(
    ...
    push_terminal_record(
        &mut state.interactive_terminal_history,
        session_id,
        record.clone(),
    );
    terminal_registry.append(session_id, record);
...
impl RuntimeKernelService for RuntimeKernelServer {
    ...
    async fn stream_terminal(
        ...
        let terminal_registry = self.terminal_registry.clone();
        let mut receiver = terminal_registry.subscribe(&task_id);
        let output = try_stream! {
            let mut last_delivered_sequence = 0u64;
            let mut last_emitted_eof = false;
            let mut stream_eof_seen = false;
            ...
            loop {
                let (task_terminal, task_finished) = {
                    let state = state.read().expect("kernel state poisoned");
                    ...
                    (task_terminal, task_finished)
                };
                if stream_eof_seen || task_terminal || task_finished {
                    if !last_emitted_eof {
                        yield TerminalChunk {
                            task_id: task_id.clone(),
                ...
                match receiver.recv().await {
                    Ok(record) => {
                        if record.sequence <= last_delivered_sequence {
                            continue;
                        }
                        if requested_stream
                            .as_ref()
                            .is_some_and(|stream| record.stream != *stream)
                        {
                            continue;
                        }
                        last_delivered_sequence = record.sequence;
                    ...
                    Err(tokio::sync::broadcast::error::RecvError::Lagged(_)) => {
                        for record in terminal_registry.replay_after(
                            &task_id,
                            requested_stream.as_deref(),
    ...
    async fn stream_terminal_session(
        ...
        let state = self.state.clone();
        let terminal_registry = self.terminal_registry.clone();
        let mut receiver = terminal_registry.subscribe(&session_id);
        let output = try_stream! {
            let mut last_delivered_sequence = 0u64;
            let mut last_emitted_eof = false;
            let mut session_eof_seen = false;
            for record in terminal_registry.replay(&session_id, None) {
                last_delivered_sequence = record.sequence;
                last_emitted_eof = record.eof;
                session_eof_seen |= record.eof;
                yield TerminalChunk {
                    task_id: task_id.clone(),
                    stream: record.stream,
                    data: record.data,
            ...
            loop {
                let session_terminal = {
                    ...
                    state
                        ...
                        .get(&session_id)
                        .is_some_and(|session| {
                            session.eof || matches!(session.status.as_str(), "closed" | "failed")
                        })
                };
                if session_eof_seen || session_terminal {
                    if !last_emitted_eof {
                        yield TerminalChunk {
                            task_id: task_id.clone(),
                            stream: "interactive".to_string(),
                ...
                match receiver.recv().await {
                    ...
                    Err(tokio::sync::broadcast::error::RecvError::Lagged(_)) => {
                        for record in terminal_registry.replay_after(&session_id, None, last_delivered_sequence) {
                            last_delivered_sequence = record.sequence;
                            last_emitted_eof = record.eof;
                            session_eof_seen |= record.eof;
                            yield TerminalChunk {
    ...
    async fn save_checkpoint(
        ...
        let (
            ...
            let state = self.state.read().expect("kernel state poisoned");
            let task =
                state.tasks.get(&payload.task_id).cloned().ok_or_else(|| {
                    Status::not_found(format!("task not found: {}", payload.task_id))
                })?;
            let environment = state
                .environments
                .get(&environment_id)
        ...
        let checkpoint_lock = self
            .checkpoint_workspace_lock(&workspace_path, &environment_id)
            .await;
        let _checkpoint_guard = checkpoint_lock.lock_owned().await;

        let checkpoint_root = environment_runtime_root.join("checkpoints");
        let checkpoint_dir = checkpoint_root.join(&checkpoint_id);
        let checkpoint_tmp_dir = checkpoint_root.join(format!(".{checkpoint_id}.tmp"));
        ...
        let tmp_manifest_path = checkpoint_tmp_dir.join("manifest.json");
        let tmp_snapshot_path = checkpoint_tmp_dir.join("workspace_snapshot.json");
        let tmp_patch_path = build_checkpoint_archive_path(&checkpoint_tmp_dir, "git.patch");
        let tmp_git_status_path =
            build_checkpoint_archive_path(&checkpoint_tmp_dir, "git_status.txt");
        let tmp_untracked_bundle_path =
            build_checkpoint_archive_path(&checkpoint_tmp_dir, "untracked.tar.gz");

        fs::create_dir_all(&checkpoint_root)
            .await
            .map_err(|error| {
                Status::internal(format!("failed to create checkpoint root: {error}"))
            })?;
        if checkpoint_tmp_dir.exists() {
            fs::remove_dir_all(&checkpoint_tmp_dir)
                .await
                .map_err(|error| {
                    Status::internal(format!(
                        "failed to remove stale checkpoint temp dir: {error}"
                    ))
                })?;
        }
        ...
        fs::create_dir_all(&checkpoint_tmp_dir)
            .await
            .map_err(|error| {
                Status::internal(format!("failed to create checkpoint temp dir: {error}"))
            })?;

        let workspace_root = PathBuf::from(&workspace_path);
        let git_capture = task::spawn_blocking(move || -> std::result::Result<_, String> {
            let git_status_text = run_git_output(
                &workspace_root,
                &["status", "--short", "--untracked-files=all"],
            )
            .map_err(|error| format!("failed to capture git status: {error}"))?;
            let patch_bytes = git_diff_bytes(&workspace_root)
                .map_err(|error| format!("failed to capture git diff: {error}"))?;
            let untracked_bundle_bytes = build_untracked_bundle_bytes(&workspace_root)
                .map_err(|error| format!("failed to capture untracked bundle: {error}"))?;
            let commit_sha = git_head_commit_sha(&workspace_root)
                .map_err(|error| format!("failed to capture git commit sha: {error}"))?;
            Ok((
                git_status_text,
                patch_bytes,
        ...
        .await
        .map_err(|error| Status::internal(format!("checkpoint capture join failure: {error}")))?;
        let (git_status_text, patch_bytes, untracked_bundle_bytes, commit_sha) =
            git_capture.map_err(Status::internal)?;
        let has_untracked_bundle = !untracked_bundle_bytes.is_empty();

        let manifest_json = serde_json::to_vec(&serde_json::json!({
            "task": TaskSnapshot::from(&task_record),
            "environment": EnvironmentSnapshot::from(&environment_record),
            "checkpoint": {
                "task_id": payload.task_id.clone(),
                "environment_id": environment_id.clone(),
        ...
        fs::write(&tmp_snapshot_path, &snapshot_json)
            .await
            .map_err(|error| {
                Status::internal(format!("failed to write checkpoint snapshot: {error}"))
            })?;
        fs::write(&tmp_patch_path, &patch_bytes)
            .await
            .map_err(|error| {
                Status::internal(format!("failed to write checkpoint patch: {error}"))
            })?;
        fs::write(&tmp_git_status_path, git_status_text.as_bytes())
            .await
            .map_err(|error| Status::internal(format!("failed to write git status: {error}")))?;
        if has_untracked_bundle {
            fs::write(&tmp_untracked_bundle_path, &untracked_bundle_bytes)
                .await
                .map_err(|error| {
                    Status::internal(format!("failed to write untracked bundle: {error}"))
                })?;
        }
        fs::rename(&checkpoint_tmp_dir, &checkpoint_dir)
            .await
            .map_err(|error| {
                Status::internal(format!("failed to publish checkpoint atomically: {error}"))
            })?;
    ...
    async fn restore_checkpoint(
        ...
        let checkpoint_lock = self
            .checkpoint_workspace_lock(&workspace_path, &checkpoint.environment_id)
            .await;
        let _checkpoint_guard = checkpoint_lock.lock_owned().await;
        let restore_result = task::spawn_blocking({
            let checkpoint = checkpoint.clone();
            let workspace_path = workspace_path.clone();
            move || -> std::result::Result<RestoreCheckpointOutcome, String> {
    ...
    async fn terminal_history_is_bounded_and_streams_incrementally() -> Result<()> {
...

        {
            ...
            state
                .terminal_history
                .insert("task-history".to_string(), Vec::new());
            server.terminal_registry.clear("task-history");
            let task = state
                .tasks
