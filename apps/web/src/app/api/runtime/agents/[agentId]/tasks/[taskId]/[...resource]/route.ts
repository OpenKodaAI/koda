...
async function parseRequestParams(
  params: Promise<{ agentId: string; taskId: string; resource: string[] }>,
) {
  const parsed = await params;
  return {
    agentId: parseSchemaOrThrow(agentIdSchema, parsed.agentId, "Invalid agent id."),
...
async function proxyJson(
  ...
  const response = await runtimeFetch(agentId, pathname, init, searchParams, {
    capability,
  });
...
async function buildTerminalAttachResponse(
  ...
  const relay = await createRuntimeRelayDescriptor({
    ...
    upstreamUrl: toAbsoluteUpstreamWsUrl(agent.runtimeBaseUrl, upstreamWs),
...
export async function POST(
  ...
  try {
    if (resourcePath === "attach/terminal") {
      ...
      return buildTerminalAttachResponse(agentId, numericTaskId, response.data, operatorToken);
    }
    ...
    if (resourcePath === "attach/browser") {
      ...
      return buildBrowserAttachResponse(agentId, numericTaskId, response.data, operatorToken);
    }
