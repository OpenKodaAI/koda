...
export function AgentDetailModal({
  ...
  const { tl } = useAppI18n();
  const presence = useAnimatedPresence(isOpen && Boolean(agentId), { agentId }, { duration: 320 });
  const renderedBotId = presence.renderedValue.agentId ?? agentId;
  const agentDisplay = renderedBotId ? agentDisplayMap[renderedBotId] : null;

  useBodyScrollLock(presence.shouldRender);
  useEscapeToClose(presence.shouldRender, onClose);

  if (!presence.shouldRender || !renderedBotId || !agentDisplay) {
    return null;
  }
      ...
      <div className="app-modal-frame z-[70] items-stretch p-3 sm:p-5 lg:p-6 xl:p-7">
        ...
        >
            ...
            <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
              ...
              <div className="flex min-w-0 flex-col gap-3 xl:items-end">
                <AgentSwitcher
                  ...
                  onAgentChange={(nextBotId) => {
                    if (nextBotId) onAgentChange(nextBotId);
                  }}
                  showAll={false}
                  showCreate={false}
                />
          ...
          <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4 sm:px-5 sm:py-5 lg:px-6 lg:py-6">
            <AgentDetailContent key={renderedBotId} agentId={renderedBotId} />
          </div>
        </div>
      </div>
    </>,
