...
import { BotCatalog } from "@/components/control-plane/catalog/bot-catalog";
import type {
...
function makeBot(
  id: string,
  displayName: string,
  organization: ControlPlaneBotSummary["organization"],
): ControlPlaneBotSummary {
  return {
    id,
...
const bots: ControlPlaneBotSummary[] = [
  makeBot("ATLAS", "ATLAS", {
    workspace_id: "workspace-product",
    workspace_name: "Produto",
    workspace_color: "#4F7CFF",
  ...
  makeBot("ORBITAL_ONE", "ORBITAL ONE", {
    workspace_id: "workspace-ops",
    workspace_name: "Operacoes",
    workspace_color: "#2EAF6D",
    squad_id: null,
    squad_name: null,
    squad_color: null,
  }),
...
describe("BotCatalog organization board", () => {
  ...
  beforeEach(() => {
    ...
    globalThis.fetch = vi.fn().mockImplementation((input, init) => {
      ...
      if (
        ...
        const baseBot = bots.find((item) => item.id === botId) ?? bots[0];
        const workspace =
  ...
  it("moves a bot between lanes using drag and drop", async () => {
    const user = userEvent.setup();
    renderCatalog();
  ...
  it("duplicates a bot and dispatches clone request", async () => {
    const user = userEvent.setup();
    renderCatalog();
