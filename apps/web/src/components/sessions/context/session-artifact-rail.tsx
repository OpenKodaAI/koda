    ...
    return (
      ...
      >
        <RoomSettingsPanel
          ...
          enabled={open}
          variant="rail"
          onArchived={onRoomArchived}
          className="flex-1"
        />
        <ThreadReplyCard messages={roomThreadMessages} variant="rail" />
      </aside>
    );
  }
...
function ThreadReplyCard({
  ...
  const content = (
    <div
      className={cn(
        "flex max-h-[260px] flex-col gap-1 overflow-y-auto pb-2",
        variant === "card" ? "px-2" : "px-0",
      )}
    >
